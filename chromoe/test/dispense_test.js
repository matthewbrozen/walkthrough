...
describe("dispense", function(){
  it("should GET all dispenses on /dispenses GET", function(done){
    api.get("/dispenses")
    .end(function(err,res){
      expect(res.status).to.equal(200);
      expect(res.type).to.equal('application/json');
      //test length of array of records must know length first
      expect(res.body.allDispenses.length).to.equal(0);
      done();
    })
  })
});
...
//PUT products test
describe("dispense", function(){
it("should PUT the product on /dispenses PUT", function(done){
  api.put("/dispenses/")
     .send({"product":"test"})
  .end(function(err,res){
    expect(res.status).to.equal(200);
    expect(res.type).to.equal('application/json');
    expect(res.body.criteriaDispenses.length).to.equal(0);
    done();
  })
})
});
...
it("should PATCH the PID on /conditions PATCH", function(done){
  ...
  .end(function(err,res){
    expect(res.status).to.equal(200);
    expect(res.type).to.equal('application/json');
    expect(res.body.patientDispenses.length).to.equal(0);
    done();
  })
})
});
